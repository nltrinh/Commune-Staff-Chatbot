...
mkdir -p ~/data/db
# Kill bất kỳ tiến trình mongod nào đang chạy để restart với cấu hình mới
...
sleep 5
mongosh --eval 'rs.initiate()' || echo "Replica Set đã được khởi tạo hoặc có lỗi nhẹ (không sao)."
...
echo "📥 Đang tải mô hình Qwen2.5:14b (Nặng ~9GB)..."
...
# 6. Khởi động Server
echo "🔥 Đang khởi động Backend Server (4 workers)..."
...
echo "🔑 Tài khoản Admin: host_admin / admin123"
echo "📜 Xem log server tại: tail -f server.log"
echo "----------------------------------------------------"
echo "💡 Lưu ý: Hãy chạy lệnh Cloudflare Tunnel để truy cập từ xa."
